...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public static void getServerList(string str)
	{
		...
		{
			// HSNR: bỏ qua mọi nguồn server (HTTP/RMS/linkDefault), luôn dùng 5
			// server HSNR khai báo tập trung trong HsnrConfig.
			str = HsnrConfig.BuildServerListString();
		}
		...
		HsnrLog.Log("SRVLIST", "getServerList parsed " + nameServer.Length + " servers, priority=" + serverPriority + " -> saveIP");
		saveIP();
		HsnrLog.Log("SRVLIST", "getServerList after saveIP done");
	}
	...
	public static void loadIP()
	{
		if (HsnrConfig.useHsnrProtocol)
		{
			// HSNR: không đọc server từ RMS cache, luôn nạp 5 server HSNR.
			getServerList(linkDefault);
			// Tự ConnectIP ở đây, KHÔNG phụ thuộc saveIP->SplashScr.loadIP (saveIP có
			// thể ném exception bị catch rỗng nuốt -> ConnectIP không bao giờ chạy -> đen).
			if (ipSelect < 0 || nameServer == null || ipSelect >= nameServer.Length)
			{
				int pick = (serverPriority >= 0 && nameServer != null && serverPriority < nameServer.Length) ? serverPriority : 0;
				SetIpSelect(pick, issave: true);
			}
			HsnrLog.Log("SRVLIST", "HSNR loadIP -> ConnectIP ipSelect=" + ipSelect + " addr=" + (nameServer != null && ipSelect < address.Length ? address[ipSelect] : "?"));
			ConnectIP();
			return;
		}
