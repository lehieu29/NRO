...
public class GameScr : mScreen, IChatable
{
	...
	public bool isMeCanAttackMob(Mob m)
	{
		...
		Char obj = findCharInMap(m.mobId);
		if (obj == null)
		{
			return true;
		}
		...
		if (Char.myCharz().isMeCanAttackOtherPlayer(obj))
		{
			return true;
		}
		return false;
	...
	private bool checkSingleClickEarly()
	{
		...
		if (mapObject != null)
		{
			...
			{
				Char obj = (Char)mapObject;
				if (obj.cTypePk != 5 && !obj.isAttacPlayerStatus())
				{
	...
	public override void paint(mGraphics g)
	{
		...
		for (int i = 0; i < vCharInMap.size(); i++)
		{
			...
			if (obj.isMabuHold && TileMap.mapID == 128)
			{
				obj.paintHeadWithXY(g, obj.cx, obj.cy, 0);
			}
		}
		...
		for (int i = 0; i < vCharInMap.size(); i++)
		{
			...
			try
			{
				obj2 = (Char)vCharInMap.elementAt(i);
			}
			catch (Exception ex)
			{
		...
		for (int i = 0; i < vCharInMap.size(); i++)
		{
			...
			try
			{
				obj4 = (Char)vCharInMap.elementAt(i);
			}
			catch (Exception ex3)
			{
		...
		for (int i = 0; i < vCharInMap.size(); i++)
		{
			...
			{
				obj5.chatInfo.paint(g, obj5.cx, obj5.cy - obj5.ch, obj5.cdir);
			}
		}
		...
		if (!isPaintOther)
		{
			...
			for (int i = 0; i < vCharInMap.size(); i++)
			{
				...
				try
				{
					obj6 = (Char)vCharInMap.elementAt(i);
				}
				catch (Exception)
				{
				}
				if (obj6 != null && obj6.isFusion && Char.isCharInScreen(obj6))
				{
					obj6.tFusion++;
					if (GameCanvas.gameTick % 3 == 0)
					{
			...
			if (Char.myCharz().clan != null)
			{
				...
				for (int i = 0; i < vCharInMap.size(); i++)
				{
					Char obj7 = (Char)vCharInMap.elementAt(i);
					if (obj7.clanID == -1 || obj7.clanID != Char.myCharz().clan.ID)
					{
						continue;
					}
					if (obj7.isOutX() && obj7.cx < Char.myCharz().cx)
					{
						int num11 = num10;
						...
						if (num8 <= num11)
						{
							mFont.tahoma_7_green.drawString(g, obj7.cName, 20, num7 - 12 + num8 * 12, mFont.LEFT, mFont.tahoma_7_grey);
							obj7.paintHp(g, 10, num7 + num8 * 12 - 5);
							num8++;
						}
					}
					else if (obj7.isOutX() && obj7.cx > Char.myCharz().cx && num9 <= num10)
					{
						mFont.tahoma_7_green.drawString(g, obj7.cName, GameCanvas.w - 25, num7 - 12 + num9 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
						obj7.paintHp(g, GameCanvas.w - 15, num7 + num9 * 12 - 5);
						num9++;
					}
				}
			}
	...
	private Char findCharVS2()
	{
		...
		{
			Char obj = (Char)vCharInMap.elementAt(i);
			if (obj.cTypePk != 0 && obj != findCharVS1())
			{
				return obj;
			}
		}
	...
	{
		Char obj = findCharInMap(playerId);
		if (obj != null)
		{
			if (typePK == 3)
			{
				startYesNoPopUp(info, new Command(mResources.OK, 2000, obj), new Command(mResources.CLOSE, 2009, obj));
			}
			if (typePK == 4)
			{
				startYesNoPopUp(info, new Command(mResources.OK, 2005, obj), new Command(mResources.CLOSE, 2009, obj));
			}
		}
	}
	...
	public void giaodich(int playerID)
	{
		...
		if (obj != null)
		{
			startYesNoPopUp(obj.cName + mResources.want_to_trade, new Command(mResources.YES, 11114, obj), new Command(mResources.NO, 2009, obj));
		}
	}
	...
	public void actionPerform(int idAction, object p)
	{
		...
		case 11113:
		{
			...
			if (obj2 != null)
			{
				Service.gI().giaodich(0, obj2.charID, -1, -1);
			}
			break;
		}
		case 11114:
		{
			...
			if (obj3 != null)
			{
				Service.gI().giaodich(1, obj3.charID, -1, -1);
			}
			break;
		}
		case 11111:
