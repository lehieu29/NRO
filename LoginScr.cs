...
public class LoginScr : mScreen, IActionListener
{
	...
	public void doLogin()
	{
		...
		{
			HsnrLog.Log("DOLOGIN", "not connected -> GameCanvas.connect() (async)");
			GameCanvas.connect();
		}
		HsnrLog.Log("DOLOGIN", "isConnected=" + Session_ME.gI().isConnected() + " connected=" + Session_ME.connected + " hsnr=" + HsnrConfig.useHsnrProtocol + " sendInfoFirst=" + HsnrConfig.sendClientInfoBeforeLogin);
		if (HsnrConfig.useHsnrProtocol && HsnrConfig.sendClientInfoBeforeLogin)
		{
			Service.gI().setClientType();
			Service.gI().sendDeviceUuid();
		}
		...
		{
			HsnrLog.Log("DOLOGIN", "Session_ME.connected=false -> show 'may chu tat' [0]");
