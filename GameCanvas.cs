...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		...
		try
		{
			...
			if ((currentScreen != serverScr || !serverScr.isPaintNewUi) && Controller.isConnectOK)
			{
				if (HsnrConfig.useHsnrProtocol)
				{
					// HSNR: KHÔNG auto-gửi setClientType ở đây. Toàn bộ chuỗi
					// setClientType -> deviceUuid -> login do LoginScr.doLogin làm chủ,
					// gửi đúng thứ tự 1 lần (giống probe đã verify). Tránh gửi lặp/sai thứ tự.
					HsnrLog.Log("CONNECT", "onConnectOK (HSNR) - skip auto setClientType, doLogin owns sequence");
					Controller.isConnectOK = false;
				}
				else if (Controller.isMain)
				{
	...
	{
		HsnrLog.Log("DISC", "onDisconnected; currentScreen=" + (currentScreen == null ? "null" : currentScreen.GetType().Name) + " isLoadingData=" + Controller.isLoadingData);
		if (Controller.isConnectionFail)
		{
			Controller.isConnectionFail = false;
		}
	...
	{
		HsnrLog.Log("FAIL", "onConnectionFail; currentScreen=" + (currentScreen == null ? "null" : currentScreen.GetType().Name) + " isLoadingData=" + Controller.isLoadingData);
