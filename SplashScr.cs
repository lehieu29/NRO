public class SplashScr : mScreen
{
	...
	public override void update()
	{
		...
		{
			isCheckConnect = true;
			HsnrLog.Log("SPLASH", "stat==30: begin loadIP");
			...
			try
			{
				...
				HsnrLog.Log("SPLASH", "loadIP done. servers=" + (ServerListScreen.nameServer == null ? "null" : ServerListScreen.nameServer.Length.ToString()) + " ip=" + GameMidlet.IP + " port=" + GameMidlet.PORT);
			}
			catch (System.Exception ex)
			{
				HsnrLog.Log("SPLASH", "loadIP EXCEPTION: " + ex.GetType().Name + " " + ex.Message);
			}
		}
